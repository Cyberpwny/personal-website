...
The production target is Cloudflare Pages. Deploy `dist/` after running `npm run build`; the generated site requires no framework preset or server-side functions.

The initial address uses Cloudflare's free `pages.dev` domain. A custom domain can be added later without changing the site structure.

## Before public deployment

1. Add the public domain as the canonical URL and Open Graph URL in `index.html`.
2. Add each public case-study anchor URL to the optional Freelancermap link field.
3. Confirm that the confidential-deployment claims remain approved for public use.
